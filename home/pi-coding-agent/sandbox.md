...
In this sandbox, `/tmp` is backed by a persistent per-project host directory, so files there survive pi restarts and can be reused across sessions for the same project.
